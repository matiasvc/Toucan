...
bool update_framebuffer_2d(Figure2D& figure_2d, const Vector2i& size) {
	if (figure_2d.framebuffer == 0 or figure_2d.framebuffer_size != size) {
		create_or_resize_framebuffer(&figure_2d.framebuffer, &figure_2d.framebuffer_color_texture, nullptr, Vector2i(std::max(size.x(), 1), std::max(size.y(), 1)));
...
void draw_lineplot_2d(Element2D& element_2d, const Matrix4f& model_to_world_matrix, const Matrix4f& world_to_camera_matrix, ToucanContext& context) {
	...
	const Matrix4f model_matrix = model_to_world_matrix * element_2d.line_plot_2d_metadata.settings.transform.transformation_matrix_3d();
...
void draw_point_2d(Element2D& element_2d, const Matrix4f& model_to_world_matrix, const Matrix4f& world_to_camera_matrix, ToucanContext& context) {
	...
	const Matrix4f model_matrix = model_to_world_matrix * element_2d.point_2d_metadata.settings.transform.transformation_matrix_3d();
...
void draw_image_2d(Element2D& element_2d, const Matrix4f& model_to_world_matrix, const Matrix4f& world_to_camera_matrix, ToucanContext& context) {
	if (element_2d.data_buffer_ptr != nullptr) { // Is there new data we need to upload?
		...
		switch (element_2d.image_2d_metadata.format) {
			case ImageFormat::RG_U8:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RG, GL_UNSIGNED_BYTE, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RG_U16:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RG, GL_UNSIGNED_SHORT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RG_U32:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RG, GL_UNSIGNED_INT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RG_F32:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RG, GL_FLOAT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RGB_U8:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RGB, GL_UNSIGNED_BYTE, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RGB_U16:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RGB, GL_UNSIGNED_SHORT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RGB_U32:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RGB, GL_UNSIGNED_INT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::RGB_F32:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RGB, GL_FLOAT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::BGR_U8:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_BGR, GL_UNSIGNED_BYTE, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::BGR_U16:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_BGR, GL_UNSIGNED_SHORT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::BGR_U32:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_BGR, GL_UNSIGNED_INT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::BGR_F32:
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_BGR, GL_FLOAT, element_2d.data_buffer_ptr);
				break;
			case ImageFormat::GRAY_U16:
				glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G, GL_RED);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, GL_RED);
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, element_2d.image_2d_metadata.width, element_2d.image_2d_metadata.height, 0, GL_RED, GL_UNSIGNED_SHORT, element_2d.data_buffer_ptr);
				break;
		}
...
void draw_element_2d(Element2D& element_2d, const Matrix4f& model_to_world_matrix, const Matrix4f& world_to_camera_matrix, ToucanContext& context) {
	...
	switch (element_2d.type) {
		case ElementType2D::LinePlot2D:
			draw_lineplot_2d(element_2d, model_to_world_matrix, world_to_camera_matrix, context);
			break;
		case ElementType2D::Point2D:
			draw_point_2d(element_2d, model_to_world_matrix, world_to_camera_matrix, context);
			break;
		case ElementType2D::Image2D:
			draw_image_2d(element_2d, model_to_world_matrix, world_to_camera_matrix, context);
			break;
	}
}

bool update_framebuffer_3d(Figure3D& figure_3d, Vector2i size) {
...
void draw_grid_3d(Element3D& element_3d, const Matrix4f& model_to_world_matrix, const Matrix4f& orientation_and_handedness_matrix, const Matrix4f& world_to_camera_matrix, const Matrix4f& projection_matrix, ToucanContext& context) {
...
void draw_axis_3d(Element3D& element_3d, const Matrix4f& model_to_world_matrix, const Matrix4f& orientation_and_handedness_matrix, const Matrix4f& world_to_camera_matrix, const Matrix4f& projection_matrix, ToucanContext& context) {
	unsigned int line_3d_shader = get_line_3d_shader(context.asset_context);
	glUseProgram(line_3d_shader);
	
	const Matrix4f model_matrix = model_to_world_matrix * orientation_and_handedness_matrix * ScaledTransform3Df(Quaternionf::Identity(), Vector3f::Zero(), Vector3f::Ones() * element_3d.axis_3d_metadata.settings.size).transformation_matrix();
...
void draw_point_3d(Element3D& element_3d, const Matrix4f& model_to_world_matrix, const Matrix4f& orientation_and_handedness_matrix, const Matrix4f& world_to_camera_matrix, const Matrix4f& projection_matrix, ToucanContext& context) {
	...
	// TODO(Matias): Use uniform buffer object to set the matrix once for all objects that are rendered
	const Matrix4f model_matrix = model_to_world_matrix * orientation_and_handedness_matrix * element_3d.point_3d_metadata.settings.transform.transformation_matrix();
...
void draw_line_3d(Element3D& element_3d, const Matrix4f& model_to_world_matrix, const Matrix4f& orientation_and_handedness_matrix, const Matrix4f& world_to_camera_matrix, const Matrix4f& projection_matrix, ToucanContext& context) {
	...
	// TODO(Matias): Use uniform buffer object to set the matrix once for all objects that are rendered
	const Matrix4f model_matrix = model_to_world_matrix * orientation_and_handedness_matrix * element_3d.line_3d_metadata.settings.transform.transformation_matrix();
	set_shader_uniform(line_3d_shader, "model", model_matrix);
	set_shader_uniform(line_3d_shader, "view", world_to_camera_matrix);
	set_shader_uniform(line_3d_shader, "projection", projection_matrix);
...
void draw_primitive_3d(Element3D& element_3d, const Matrix4f& model_to_world_matrix, const Matrix4f& orientation_and_handedness_matrix, const Matrix4f& world_to_camera_matrix, const Matrix4f& projection_matrix, ToucanContext& context) {
	if (element_3d.data_buffer_ptr != nullptr) {
		...
		element_3d.primitive_3d_metadata.vertex_data_ptr = reinterpret_cast<Primitive3D*>(element_3d.data_buffer_ptr);
		element_3d.data_buffer_ptr = nullptr;
	}
	...
	for (int primitive_index = 0; primitive_index < element_3d.primitive_3d_metadata.number_of_primitives; ++primitive_index) {
		...
		// We multiply with the orientation and handedness matrix twice to ensure that the primitives are oriented around the "up" axis, no matter which axis that is.
		const Matrix4f model_matrix = model_to_world_matrix * orientation_and_handedness_matrix * primitive.scaled_transform.transformation_matrix() * orientation_and_handedness_matrix.transpose();
...
void draw_element_3d(Element3D& element_3d, const Matrix4f& model_to_world_matrix, const Matrix4f& orientation_and_handedness_matrix, const Matrix4f& world_to_camera_matrix, const Matrix4f& projection_matrix, ToucanContext& context) {
	switch (element_3d.type) {
		case ElementType3D::Grid3D:
			draw_grid_3d(element_3d, model_to_world_matrix, orientation_and_handedness_matrix, world_to_camera_matrix, projection_matrix, context);
			break;
		case ElementType3D::Axis3D:
			draw_axis_3d(element_3d, model_to_world_matrix, orientation_and_handedness_matrix, world_to_camera_matrix, projection_matrix, context);
			break;
		case ElementType3D::Point3D:
			draw_point_3d(element_3d, model_to_world_matrix, orientation_and_handedness_matrix, world_to_camera_matrix, projection_matrix, context);
			break;
		case ElementType3D::Line3D:
			draw_line_3d(element_3d, model_to_world_matrix, orientation_and_handedness_matrix, world_to_camera_matrix, projection_matrix, context);
			break;
		case ElementType3D::Primitive3D:
			draw_primitive_3d(element_3d, model_to_world_matrix, orientation_and_handedness_matrix, world_to_camera_matrix, projection_matrix, context);
			break;
	}
}

void draw_axis_gizmo_3d(const RigidTransform3Df& camera_transform, const Vector2i& framebuffer_size, const Matrix4f& orientation_and_handedness_matrix, ToucanContext& context) {
	...
	const Matrix4f world_to_camera_matrix = camera_transform.transformation_matrix();
	const Matrix4f projection_matrix = create_3d_projection_matrix<float>(0.01f, 150.0f, 4.0f * size, Vector2i(size, size));
	
	set_shader_uniform(mesh_3d_shader, "view", world_to_camera_matrix);
	set_shader_uniform(mesh_3d_shader, "projection", projection_matrix);
	set_shader_uniform(mesh_3d_shader, "light_vector", Vector3f(1.0f, 1.5f, 1.8f).normalized());
	
	{
		set_shader_uniform(mesh_3d_shader, "color", Color(0.8f, 0.8f, 0.8f));
		const Matrix4f model_matrix = ScaledTransform3Df(Quaternionf::Identity(), Vector3f::Zero(), Vector3f::Ones()).transformation_matrix() * orientation_and_handedness_matrix;
		set_shader_uniform(mesh_3d_shader, "model", model_matrix);
	...
	{ // X-axis
		set_shader_uniform(mesh_3d_shader, "color", Color::Red());
		const Matrix4f model_matrix = orientation_and_handedness_matrix * ScaledTransform3Df(
	...
	{ // Y-axis
		set_shader_uniform(mesh_3d_shader, "color", Color::Green());
		const Matrix4f model_matrix = orientation_and_handedness_matrix * ScaledTransform3Df(
	...
	{ // Z-axis
		set_shader_uniform(mesh_3d_shader, "color", Color::Blue());
		const Matrix4f model_matrix = orientation_and_handedness_matrix * ScaledTransform3Df(
...
Rectangle get_lineplot_2d_data_bounds(const Element2D& element_2d, const RigidTransform2Df& local_transform) {
	assert(element_2d.type == ElementType2D::LinePlot2D);
	
	const ScaledTransform2Df& data_transform = element_2d.line_plot_2d_metadata.settings.transform;
	auto* point_2d_data_ptr = reinterpret_cast<Vector2f*>(element_2d.data_buffer_ptr);
	...
	for (int point_2d_data_index = 0; point_2d_data_index < element_2d.line_plot_2d_metadata.number_of_points; ++point_2d_data_index) {
		const Vector2f local_point = point_2d_data_ptr[point_2d_data_index];
		const Vector2f global_point = local_transform * (data_transform * local_point);
	...
	return Rectangle(Vector2f(min_x, min_y), Vector2f(max_x, max_y));
}

Rectangle get_point_2d_data_bounds(const Element2D& element_2d, const RigidTransform2Df& local_transform) {
	assert(element_2d.type == ElementType2D::Point2D);
	
	const ScaledTransform2Df& data_transform = element_2d.point_2d_metadata.settings.transform;
	auto* point_2d_data_ptr = reinterpret_cast<Point2D*>(element_2d.data_buffer_ptr);
	...
	for (int point_2d_data_index = 0; point_2d_data_index < element_2d.point_2d_metadata.number_of_points; ++point_2d_data_index) {
		const Vector2f local_point = point_2d_data_ptr[point_2d_data_index].position;
		const Vector2f global_point = local_transform * (data_transform * local_point);
		const auto& x = global_point.x();
		const auto& y = global_point.y();
...
Rectangle get_image_2d_data_bounds(const Element2D& element_2d, const RigidTransform2Df& local_transform) {
	assert(element_2d.type == ElementType2D::Image2D);
	...
	const std::array<Vector2f, 4> image_corners = {
			local_transform * Vector2f(0.0f, 0.0f),
			local_transform * Vector2f(static_cast<float>(image_draw_width), 0.0f),
			local_transform * Vector2f(0.0f, static_cast<float>(image_draw_height)),
			local_transform * Vector2f(static_cast<float>(image_draw_width), static_cast<float>(image_draw_height))
	};
...
void update_figure_2d_view_data(Figure2D& figure_2d) {
	if (figure_2d.elements.empty()) { // There are no elements in the figure so set a reasonable default view
		figure_2d.view = Rectangle(Vector2f(-5.0f, -5.0f), Vector2f(5.0f, 5.0f));
		return;
	}
	
	std::vector<Rectangle> data_bounds_vec;
	...
	for (auto& element_2d : figure_2d.elements) {
		...
		if (element_2d.data_buffer_ptr == nullptr) { // Use view from cache
			...
			const RigidTransform2Df& local_transform = element_2d.pose;
	...
	Rectangle figure_data_bounds = data_bounds_vec.front();
	
	for (const auto& data_bounds : data_bounds_vec) {
		figure_data_bounds = get_union_of_rectangles(figure_data_bounds, data_bounds);
	}
	...
	figure_2d.view = get_resized_rectangle(figure_data_bounds, Vector2f(0.05f*figure_data_bounds.width(), 0.05f*figure_data_bounds.height()));
}
...
void draw_figure_2d_list(std::list<Figure2D>& figures, ToucanContext& context) {
	...
	for (auto& figure_2d : figures) {
		...
		if (ImGui::Begin(figure_2d.name.c_str())) {
			...
			const Vector2i figure_draw_size(
			...
			if (scroll_x != 0) {
				const auto mouse_plot_value_x = remap(io.MousePos.x, plot_min.x, plot_max.x,
			...
			if (scroll_y != 0) {
				...
				if (figure_2d.settings.y_axis_direction == YAxisDirection::UP) {
					mouse_plot_value_y = remap(io.MousePos.y, plot_min.y, plot_max.y, figure_2d.view.max.y(), figure_2d.view.min.y());
				} else {
					mouse_plot_value_y = remap(io.MousePos.y, plot_min.y, plot_max.y, figure_2d.view.min.y(), figure_2d.view.max.y());
				}
				if (scroll > 0) {
					for (int scroll_index = 0; scroll_index < scroll; scroll_index++) {
			...
			const bool framebuffer_was_updated = update_framebuffer_2d(figure_2d, figure_draw_size);
			const bool elements_has_new_data = std::any_of(
					figure_2d.elements.cbegin(), figure_2d.elements.cend(),
					[](const Element2D& element) { return element.data_buffer_ptr != nullptr; }
			);
			...
			if (view_changed_this_frame or framebuffer_was_updated or elements_has_new_data) {
				...
				const Matrix4f view_matrix = create_2d_view_matrix(figure_2d.view, figure_2d.settings.y_axis_direction);
				for (auto& element : figure_2d.elements) {
					const auto& model_to_world = element.pose.transformation_matrix_3d();
					draw_element_2d(element, model_to_world, view_matrix, context);
				}
			...
			const auto x_ticks_position = data_to_pixel(x_tick_values, x_axis_from_value, x_axis_to_value, axis_x_min.x, axis_x_max.x);
			...
			if (figure_2d.settings.y_axis_direction == YAxisDirection::UP) {
				y_ticks_positions = data_to_pixel(y_ticks_values, y_axis_from_value, y_axis_to_value, axis_y_max.y, axis_y_min.y);
			} else {
				y_ticks_positions = data_to_pixel(y_ticks_values, y_axis_from_value, y_axis_to_value, axis_y_min.y, axis_y_max.y);
			}
		...
		if (ImGui::Begin(figure_3d.name.c_str())) {
			...
			if (plot_held) {
				if (ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
					figure_3d.camera.orbit(-0.005f*Vector2f(io.MouseDelta.x, io.MouseDelta.y));
					view_was_changed = true;
				}
				if (ImGui::IsMouseDragging(ImGuiMouseButton_Right)) {
					figure_3d.camera.move(0.001f*Vector2f(io.MouseDelta.x, io.MouseDelta.y));
					view_was_changed = true;
				}
			}
			...
			// ***** Drawing *****
			const bool framebuffer_was_updated = update_framebuffer_3d(figure_3d, figure_draw_size);
			const bool elements_has_new_data = std::any_of(
					figure_3d.elements.cbegin(), figure_3d.elements.cend(),
					[](const Element3D& element) { return element.data_buffer_ptr != nullptr; }
			);
			...
			if (view_was_changed or framebuffer_was_updated or elements_has_new_data) {
				...
				const Matrix4f world_to_camera_matrix = figure_3d.camera.get_pose().inverse().transformation_matrix();
				const Matrix4f orientation_and_handedness_matrix = create_3d_orientation_and_handedness_matrix(figure_3d.settings.orientation, figure_3d.settings.handedness);
				
				const auto near_clip = figure_3d.settings.near_clip;
				const auto far_clip = figure_3d.settings.far_clip;
				const Matrix4f projection_matrix = create_3d_projection_matrix<float>(near_clip, far_clip, 1024, figure_draw_size);
				
				for (auto& element : figure_3d.elements) {
					const auto model_to_world_matrix = element.pose.transformation_matrix();
					draw_element_3d(element, model_to_world_matrix, orientation_and_handedness_matrix, world_to_camera_matrix, projection_matrix, context);
				}
				
				if (figure_3d.settings.gizmo_enabled) {
					draw_axis_gizmo_3d(figure_3d.camera.get_orbit_pose(100.0f).inverse(), figure_draw_size, orientation_and_handedness_matrix, context);
				}
		...
		if (ImGui::Begin(input_window.name.c_str())) {
			...
			for (auto& input_element : input_window.elements) {
				switch (input_element.type) {
					...
					case ElementInputType::SLIDER_FLOAT : {
					...
					case ElementInputType::SLIDER_FLOAT2 : {
					...
					case ElementInputType::SLIDER_FLOAT3 : {
					...
					case ElementInputType::SLIDER_FLOAT4 : {
					...
					case ElementInputType::SLIDER_INT : {
					...
					case ElementInputType::SLIDER_INT2 : {
					...
					case ElementInputType::SLIDER_INT3 : {
					...
					case ElementInputType::SLIDER_INT4 : {
					...
					case ElementInputType::COLOR_PICKER : {
